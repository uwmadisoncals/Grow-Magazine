...
<div class="Center">
		...
		<?php if (have_posts()): ?>
			<?php while (have_posts()): the_post(); ?>
					...
					<?php if (has_excerpt($post_id)){?>
                        ...
                        <?php } ?>					
                    <p id="BlogDate"><!--<?php _e('Posted By', 'wp-print'); ?> <u><?php the_author(); ?></u> -->
                    <?php 
					//get author
					$author =  get_post_custom_values('Author', $post_id);
							 if (count($author)>0){
							 	echo "By ".$author[0];
	...
	<!--				<?php _e('On', 'wp-print'); ?> <?php the_time(sprintf(__('%s @ %s', 'wp-print'), get_option('date_format'), get_option('time_format'))); ?> <?php _e('In', 'wp-print'); ?> <?php print_categories('<u>', '</u>'); ?> | <u><a href='#comments_controls'><?php print_comments_number(); ?></a></u>--></p>
